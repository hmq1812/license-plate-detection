...
class YOLO(object):
    _defaults = {
        "model_path": 'model_data/yolo_12000.h5',
    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            # Add 10% padding
            h_padding = abs(xmin - xmax)/20
            v_padding = abs(ymin - ymax)/20

            number_plate = image.crop((xmin - min(h_padding, xmin), ymin - min(v_padding, ymin),
                                       xmax + min(h_padding, abs(xmax - image.size[0])),
                                       ymax + min(v_padding, abs(ymax - image.size[1]))))
